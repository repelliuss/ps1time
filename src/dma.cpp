...
DMA::DMA() {
  ...
  store32(data, 0x07654321, reg_control);
}

// is an interrupt active?
bool DMA::irq_active() { return bit(data[reg_interrupt], 31); }
...
void DMA::set_interrupt(u32 val) {
  ...
  u8 channels_interrupt_ack_status = bits_in_range(data[reg_interrupt], 24, 30);
