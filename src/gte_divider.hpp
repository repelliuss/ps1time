#pragma once

#include "types.hpp"

static constexpr u8 UNR_TABLE[0x101] = {
    ,
};

namespace gte {

constexpr u32 reciprocal(u16 divisor) {
    u32 index = ((divisor & 0x7fff) + 0x40) >> 7;

    i32 factor = static_cast<i32>(UNR_TABLE[index]) + 0x101;

    i32 d = (divisor | 0x8000);

    i32 tmp = ((d * -factor) + 0x80) >> 8;

    u32 r = ((factor * (0x20000 + tmp)) + 0x80) >> 8;

    return r;
}

constexpr int leading_zeros16(u16 x) {
  int n = 16;
  unsigned y = x >> 16;
  y = x >> 8;
  if (y != 0) {
    n = n - 8;
    x = y;
  }
  y = x >> 4;
  if (y != 0) {
    n = n - 4;
    x = y;
  }
  y = x >> 2;
  if (y != 0) {
    n = n - 2;
    x = y;
  }
  y = x >> 1;
  if (y != 0)
    return n - 2;
  return n - x;
}

constexpr u32 divide(u16 numerator, u16 divisor) {
  u32 shift = leading_zeros16(divisor);

  u64 n = static_cast<u64>(numerator) << shift;
  u16 d = divisor << shift;

  u64 rcprcl = reciprocal(d);

  u64 res = (n * rcprcl + 0x8000) >> 16;

  if (res <= 0x1ffff) {
    return res;
  } else {
    return 0x1ffff;
  }
}

}
